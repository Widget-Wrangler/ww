...
# Set up custom forms on questions list
$context = Get-SPOContext
$context.Load($list.ContentTypes)
$context.ExecuteQuery()
foreach ($contentType in $list.ContentTypes) {
    if ($contentType.Name -eq "Item") {
        $contentType.NewFormUrl = $spAppPath + "/ListNewForm.aspx"
        $contentType.DisplayFormUrl = $spAppPath + "/ListDisplayForm.aspx"
        $contentType.EditFormUrl = $spAppPath + "/ListEditForm.aspx"
